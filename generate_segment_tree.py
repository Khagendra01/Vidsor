...
class SegmentTreeGenerator:
    def __init__(self, video_path: str, tracking_json_path: str, tracker: str = "bytetrack"):
    ...
    def process_llava(self, blip_descriptions: List[str], detection_summary: str) -> Dict:
        ...
        # Combine BLIP descriptions and detection info
        prompt = f"""Based on these image descriptions and object detections, provide a unified detailed description of this 1-second scene:
...
Provide a comprehensive description covering the entire second."""
    ...
    def process_second(self, second_idx: int, frames_data: List[Dict], video_fps: float) -> Dict:
        ...
        for group_idx, frame_num, result in blip_results:
            # Only access detection_groups if the group exists
            if group_idx < len(detection_groups):
        ...
        # Create detection summary for LLaVA
        all_tracks = set()
        ...
        llava_result = self.process_llava(blip_texts, detection_summary)
...
def main():
    ...
    generator = SegmentTreeGenerator(args.video, args.tracking, args.tracker)
