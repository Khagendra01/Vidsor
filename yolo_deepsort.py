...
# Run tracking on video with Bot-SORT (enhanced DeepSORT)
# Bot-SORT uses appearance features (embeddings) + motion prediction + camera motion compensation
# It's similar to DeepSORT but with improvements
print("Running YOLO with Bot-SORT tracking (enhanced DeepSORT)...")
print("Note: Bot-SORT uses appearance features, so it may be slower but more accurate than ByteTrack")
print("Available trackers in Ultralytics: bytetrack.yaml, botsort.yaml, ocsort.yaml")
results = model.track(
    source="kbc.mp4", 
    tracker="botsort.yaml",  # Bot-SORT: DeepSORT + camera motion compensation + IoU
...
output_data = {
    "video": "kbc.mp4",
    "tracker": "Bot-SORT",  # Bot-SORT: Enhanced DeepSORT with camera motion compensation
...
# Save JSON to file
output_file = "kbc_yolo_deepsort.json"
